...
static int16_t curr_adc_mean = 0;    // ACD 采样结果平均值
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  curr_adc_mean = adc_mean / ADC_NUM_MAX;    // 保存平均值
  
  HAL_ADC_Start_DMA(&ADC_Handle, (uint32_t*)&curr_adc_buff, ADC_NUM_MAX);// 开始 ADC 采样
}
