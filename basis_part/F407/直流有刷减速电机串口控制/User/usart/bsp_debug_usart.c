...
{
  HAL_USART_Transmit(&UartHandle, &str, 1, 1000);
}
...
void Usart_SendString(uint8_t *str)
{
  ...
  do 
  {
      HAL_USART_Transmit(&UartHandle,(uint8_t *)(str + k) ,1,1000);
      k++;
  } while(*(str + k)!='\0');
}
...
int fputc(int ch, FILE *f)
{
	...
	HAL_USART_Transmit(&UartHandle, (uint8_t *)&ch, 1, 1000);	
...
int fgetc(FILE *f)
{
	...
	int ch;
	HAL_USART_Receive(&UartHandle, (uint8_t *)&ch, 1, 1000);	
	return (ch);
}
/*********************************************END OF FILE**********************/
